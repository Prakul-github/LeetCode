    ...
    public class NQUeensProblem
    {
        ...
        public IList<IList<string>> SolveNQueens(int n)
        {
            ...
            foreach (int[,] solution in solutions)
            {
                ...
                foreach (IList<string> existingSolution in formattedSolution)
                {
                    //StringBuilder str = 
                    if (strSolution.Equals(existingSolution))
                    {
                        solutionExists = true;
                        break;
                    }
                }
                if (solutionExists == false) formattedSolution.Add(strSolution);
        ...
        public bool CheckDuplicates(int[,] chessBoard)
        {
            return false;
        }
        ...
        public IList<int[,]> GetPossibleSolutions(int n)
        {
            ...
            for (int row = 0; row < n; row++)
            {
                for (int col = 0; col < n; col++)
                {
                    ...
                    int[,] solution = MarkNQueens(chessBoard, row, col, n, (n * n) - 1);
            ...
            else
            {
                if (chessBoard[currentRow, currentCol] == 1)
                {
                    ...
                    for (int row = 1; row < n; row++)
                    {
                        ...
                        {
                            if (chessBoard[(currentRow + row) % n, (currentCol + col) % n] == 0)
                            {
                                currentRow = (currentRow + row) % n;
